...
\begin{proof}
    ...
    % We now show that as we subdivide, the space-frequency product of the
    % resulting blocks shrinks sufficiently fast that the cost of NUFFTs on
    % smaller blocks does not contribute to the asymptotic cost. Consider a
    % mixed block corresponding to a rectangle $[\frac{z}{\Omega}, \Omega]
    % \times [\frac{z}{R}, R]$. If we subdivide this block at some point
    % $(\omega, \frac{z}{\omega})$, then the largest possible space-frequency
    % product of the resulting lower-right block is \begin{equation}
    % \max_{\frac{z}{R} \leq \omega \leq \Omega} \ (\Omega - \omega)\left(R -
    % \frac{z}{\omega}\right) = \Omega R \left( 1 - \sqrt{\frac{z}{\Omega R}}
    % \right)^2 \leq \Omega R \end{equation} which is achieved by splitting at
    % $\omega = \sqrt{\frac{\Omega z}{R}}$, i.e. at the midpoint in log space
    % between $\frac{z}{R}$ and $\Omega$. This yields two new mixed blocks
    % $[\omega, \Omega] \times [\frac{z}{\Omega}, \frac{z}{\omega}]$ and
    % $[\frac{z}{R},\omega] \times [\frac{z}{\omega}, R]$. The largest possible
    % space-frequency products of their lower-right blocks are 
