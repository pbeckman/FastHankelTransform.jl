...
\begin{theorem}
    ...
    the computational cost of computing the NUFHT to tolerance $\epsilon$ at $n$
    frequencies from $m$ distances with local expansion order $L$ and asymptotic
    expansion order $M$ is $\bO\Big((L + M)(m + n\log n) \log \min(n,m) \Big)$.
\end{theorem}

\begin{proof}
    If $\omega_j r_k \leq z$ for all $j=1,\dots,n$ and $k=1,\dots,m$ then only
    the low-rank local expansion is used, resulting in $\bO(Lm + Ln)$
    complexity. If instead $\omega_j r_k > z$ everywhere, then only the
    asymptotic expansion is used, giving $\bO(Mm + Mn\log n)$ complexity.
    ...
    n_{\ell,b} = n$. An analogous fact holds for the asymptotic blocks. The number of levels $N_\ell$ scales like $\log\min(n,m)$. Then the
    total cost of local evaluation is 
    \begin{align}
        \sum_{\ell=1}^{N_\ell} \sum_{b=1}^{2^{\ell-1}} \bO\left(Lm_{\ell,b}^{(\text{loc})} + Ln_{\ell,b}^{(\text{loc})}\right)
        &= \sum_{\ell=1}^{N_\ell} \bO\left(Lm + Ln\right) \\
    ...
    \begin{align}
        \sum_{\ell=1}^{N_\ell} \sum_{b=1}^{2^{\ell-1}} \bO\left(Mm_{\ell,b}^{(\text{asy})} + Mn_{\ell,b}^{(\text{asy})} \log n_{\ell,b}^{(\text{asy})}\right)
        &= \sum_{\ell=1}^{N_\ell} \bO\left(Mm + Mn \log n\right) \\
        &= \bO\Big(M(m + n\log n) \log \min(n,m) \Big).
    \end{align}
    ...
    Therefore the total direct evaluation cost is $\bO(m + n)$. Summing the cost of local, asymptotic, and direct evaluation gives the result.
\end{proof}

Note that if we treat the expansion orders $L$ and $M$ as constants and consider
the square case $n=m$, this complexity simplifies to $\bO(n\log^2n)$, which agrees with the special cases studied in \cite{townsend2015fast} up to a factor of $\log\log n$, but is applicable to fully nonuniform $\omega_j$ and $r_k$.