...
In typical applications the maximum point $r_n$ is fixed by, for example, the
...
\begin{remark}
  ...
  % butterfly property of the Hankel transform~\cite{oneil2010algorithm}.
  % However, the evaluation of local blocks is neither an asymptotic nor
  % practical bottleneck, so we do not pursue an optimal-rank expansion in this
  % regime.
\end{remark}
