...
%\begin{align} \beta_{j\ell} (\Delta + \kappa^2) \psi_{j\ell}(r, \theta) =
%  \beta_{j\ell} (\lambda_{j\ell} + \kappa^2) \psi_{j\ell}(r, \theta) &=
%  \alpha_{j\ell} \psi_{j\ell}(r, \theta) \end{align} which implies
\begin{equation}
  \beta_{j\ell} 
...
\begin{figure}[t]
  ...
  \begin{subfigure}[T]{0.3\textwidth}
    \begin{tikzpicture}
      \draw (0, 0) node[inner sep=0] {\includegraphics[width=\textwidth,
      trim={2cm 1cm 0.1cm 0.2cm}, clip]{./figures/helmholtz_u_kappa25.pdf}};
      \draw[fill=none, thick](-0.44, -0.1) circle (1.48) node {}; \draw[](-0.44,
      1.8) node {\large $u$};
    \end{tikzpicture}
  \end{subfigure} 
