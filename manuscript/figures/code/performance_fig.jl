using FastHankelTransform, FastGaussQuadrature, BenchmarkTools, Plots, Plots.Measures, LaTeXStrings, Printf, DelimitedFiles, Random, LinearAlgebra, JLD
...
max_errs_all = [maximum(abs.(gs_true[j,t] - gs_nufht[i,j,t])) / maximum(abs.(gs_true[j])) for j in eachindex(ns_acc), i in eachindex(tols), t=1:n_trials]
